...
class self_Downsample(nn.Module):
    def __init__(self, nIn, nOut, stride):
        super(self_Downsample, self).__init__()
...
class ResNetBasicblock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):           #inplanes:输入特征矩阵的深度     planes:输出特征矩阵的深度
        super(ResNetBasicblock, self).__init__()

        self.conv_a = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn_a = nn.BatchNorm2d(planes)                       #使用BN便不再使用偏置bias

        self.conv_b = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn_b = nn.BatchNorm2d(planes)

        self.downsample = downsample            #为了给输入特征矩阵的高和宽做降维
...
class GeneralizedResNet_cifar(nn.Module):
    def __init__(self, block, depth, channels=3):
        ...
        self.stage_3 = self.self_make_layer(block, 64,2,2)                                          #自己加的
    ...
    def self_make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = self_Downsample(self.inplanes, planes * block.expansion, 1)
    ...
    def forward(self, x):
        ...
        x_3 = self.stage_3(x_2)    # [bs, 64, 16, 16]         #自己加的
        return x_3
    
class SpecializedResNet_cifar(nn.Module):
    def __init__(self, block, depth, inplanes=64, feature_dim=128):
        super(SpecializedResNet_cifar, self).__init__()
        self.inplanes = inplanes
        self.feature_dim = feature_dim
        layer_blocks = (depth - 2) // 6
        self.final_stage = self._make_layer(block, 128, layer_blocks, 2)
        self.avgpool = nn.AvgPool2d(8)             #8*8的平均池化
    ...
    def forward(self, base_feature_map):
        ...
        features = pooled.view(pooled.size(0), -1) #bs x 64          展平了后两维
        return features
