...
def setup_parser():
    ...
    parser.add_argument('--init_cls', '-init', type=int, default=100)  # 10
    parser.add_argument('--increment', '-incre', type=int, default=0)  # 10
    ...
    parser.add_argument('--convnet_type', '-net', type=str, default='memo_resnet32',
                        choices=['memo_resnet34_imagenet', 'memo_resnet32'])
    parser.add_argument('--prefix', '-p', type=str, help='exp type', default='fair',
    ...
    parser.add_argument('--num_classes', type=int, default=100)  # 10                #数据集的总类别数，由数据集决定。
    parser.add_argument('--GivenEw', type=bool, default=True)
    parser.add_argument('--reg_lam', type=float, default=0.0)
    parser.add_argument('--distill_weight', type=float, default=0.25)  # 新增参数
    parser.add_argument('--criterion', type=str, default="reg_dot_loss")

    # configer
    parser.add_argument('--head_class_idx', nargs='+', type=int, default=[0, 50])
    parser.add_argument('--med_class_idx', nargs='+', type=int, default=[50, 80])
    parser.add_argument('--tail_class_idx', nargs='+', type=int, default=[80, 100])
