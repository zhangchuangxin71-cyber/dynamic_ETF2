...
class BaseNet(nn.Module):
    ...
    @property     #使方法可以按照属性的方式调用   例如：obj.feature_dim   返回值为  self.convnet.out_dim
...
class AdaptiveNet(nn.Module):
    def __init__(self, convnet_type, pretrained):
        ...
        self.convnet_type=convnet_type
        self.TaskAgnosticExtractor , _ = get_convnet(convnet_type, pretrained) #Generalized blocks   得到basenet和adaptnet
        self.TaskAgnosticExtractor.train()          #self.TaskAgnosticExtractor为basenet
    ...
    def forward(self, x):
        base_feature_map = self.TaskAgnosticExtractor(x)
        features = [extractor(base_feature_map) for extractor in self.AdaptiveExtractors]
                        #这里需要一个MLP做维度调整，使扩展网络提取的特征做拼接之后再通过MLP做维度变换，变为ETF需要的维度。
                                      #在这里，当做网络结构高层扩展之后，每个高层特征分别提取出一个128*128维度的向量，此时，应该如何将每个高层网络提取出的特征进行融合呢？？
        # if len(features) > 1:
        #     fe = features[0]
        ...
        # else:
        #     fe = features
        #     fe = torch.cat(fe, 1)  #这里为需要的特征维度 128*128
        #                                        #这里可以直接通过注释下一句，来使网络没有fc层。
    ...
    def update_fc(self,nb_classes):
        ...
        if len(self.AdaptiveExtractors)==0:
            ...
            self.AdaptiveExtractors.append(_new_extractor)                #这里将新的模型骨干块与旧的模型骨干块拼接起来。
            self.AdaptiveExtractors[-1].load_state_dict(self.AdaptiveExtractors[-2].state_dict())          #这里，因为高层模型使用的结构是相同的，所以可以直接把上次训练得到的高层模型参数复制给新的高层模型参数。

        if self.out_dim is None:
            logging.info(self.AdaptiveExtractors[-1])
            self.out_dim=self.AdaptiveExtractors[-1].feature_dim
        fc = self.generate_fc(self.feature_dim, nb_classes)    ###         #这里的生成fc，在使用ETF分类器时不需要
        if self.fc is not None:                                  #将之前训练得到的分类器参数保留，例如，之前训练的前10个类，则  fc.weight.data[:10,:self.feature_dim-self.out_dim] = weight
            nb_output = self.fc.out_features
            weight = copy.deepcopy(self.fc.weight.data)
            bias = copy.deepcopy(self.fc.bias.data)
            fc.weight.data[:nb_output,:self.feature_dim-self.out_dim] = weight
    ...
    def weight_align(self, increment):                  #新旧权重块的对齐。
        weights=self.fc.weight.data
        newnorm=(torch.norm(weights[-increment:,:],p=2,dim=1))
        oldnorm=(torch.norm(weights[:-increment,:],p=2,dim=1))
        meannew=torch.mean(newnorm)
        meanold=torch.mean(oldnorm)
        gamma=meanold/meannew
        print('alignweights,gamma=',gamma)          #此处的特征对其是针对fc层而言，如何将对齐参数应用于ETF分类器？
                                                        #解   拿到ETF分类器的fc.weight.data，乘以gamma
        self.fc.weight.data[-increment:,:]*=gamma
