...
def get_convnet(convnet_type, pretrained=False):
    ...
    # AUC (cifar)
        ...
        g_blocks, s_blocks = memo_conv2_cifar()
        return g_blocks, s_blocks
    elif name == 'memo_resnet14_cifar':
        g_blocks, s_blocks = memo_resnet14_cifar()
        return g_blocks, s_blocks
    elif name == 'memo_resnet20_cifar':
        g_blocks, s_blocks = memo_resnet20_cifar()
        return g_blocks, s_blocks
    elif name == 'memo_resnet26_cifar':
        g_blocks, s_blocks = memo_resnet26_cifar()
        return g_blocks, s_blocks
    # AUC (imagenet)
    elif name == 'conv4':
...
class BaseNet(nn.Module):
    ...
    # 修复后的 load_checkpoint 方法
    def load_checkpoint(self, args):
        if args["init_cls"] == 50:
            # 关键修改：使用 args["init_cls"] 作为 B 的值，args["increment"] 作为 Inc 的值
            pkl_name = "{}_{}_{}_B{}_Inc{}".format(
                args["dataset"],
                args["seed"],
                args["convnet_type"],
                args["init_cls"],  # 原代码为 0，改为 args["init_cls"]
                args["increment"],  # 原代码为 args["init_cls"]，改为 args["increment"]
            )
...
class CosineIncrementalNet(BaseNet):
    ...
    def generate_fc(self, in_dim, out_dim):
        if self.fc is None:
            return CosineLinear(in_dim, out_dim, self.nb_proxy, to_reduce=True)
        else:
            prev_out_features = self.fc.out_features // self.nb_proxy
            return SplitCosineLinear(in_dim, prev_out_features, out_dim - prev_out_features, self.nb_proxy)
...
class IncrementalNetWithBias(BaseNet):
    ...
    def get_bias_params(self):
        return [layer.get_params() for layer in self.bias_layers]
...
class DERNet(nn.Module):
    ...
    @property
    def feature_dim(self):
        return self.out_dim * len(self.convnets) if self.out_dim else 0
...
class FOSTERNet(nn.Module):
    ...
    def load_checkpoint(self, args):
        if args["init_cls"] == 50:
            # 关键修改：使用 args["init_cls"] 和 args["increment"]
            pkl_name = "{}_{}_{}_B{}_Inc{}".format(
                args["dataset"],
                args["seed"],
                args["convnet_type"],
...
class AdaptiveNet(nn.Module):
    def __init__(self, convnet_type, pretrained):
        ...
        self.convnet_type = convnet_type
        self.TaskAgnosticExtractor, _ = get_convnet(convnet_type, pretrained)
    ...
    @property
    def feature_dim(self):
        return self.out_dim * len(self.AdaptiveExtractors) if self.out_dim else 0
    ...
    def forward(self, x):
        ...
        if len(features) > 1:
            ...
            fe = torch.cat(features, 1)
        return fe, base_feature_map, features

    def update_fc(self, nb_classes):
        _, _new_extractor = get_convnet(self.convnet_type)
        if not self.AdaptiveExtractors:
    ...
    def load_checkpoint(self, args):
        if args["init_cls"] == 50:
            ...
            checkpoint_name = f"checkpoints/finetune_{args['csv_name']}_0.pkl"
        checkpoint_name = checkpoint_name.replace("memo_", "")  # 按需保留或删除此行
        model_infos = torch.load(checkpoint_name)
